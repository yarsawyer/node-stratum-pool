...
var StratumClient = function(options){
    ...
    function handleAuthorize(message, replyToSocket){
        ...
        console.log(_this.workerPass, _this.workerPass.indexOf("d=") !== -1);
        console.log(_this.workerPass.slice(_this.workerPass.indexOf("d=")+2), _this.workerPass.slice(_this.workerPass.indexOf("d=")+2).split(','), _this.workerPass.slice(_this.workerPass.indexOf("d=")+2).split(',')[0]);
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
            ...
            console.log(_this.fixed_diff, options.ports[options.socket.localPort].minDiff, options.ports[options.socket.localPort].maxDiff)
            console.log(options.socket.localPort)
            console.log(options)
            if (_this.fixed_diff && _this.fixed_diff > options.ports[options.socket.localPort].minDiff && _this.fixed_diff < options.ports[options.socket.localPort].maxDiff)
            {
                _this.sendDifficulty(_this.fixed_diff);
            }
            else if (typeof (options.ports[options.socket.localPort]) !== 'undefined' && options.ports[options.socket.localPort].diff) {
                _this.sendDifficulty(options.ports[options.socket.localPort].diff);
            } else {
                _this.sendDifficulty(8);
            }
